...
import {
  BarberAndAccountDTO,
  BarberDTO,
  barberAndAccountBodyRequesSchema,
} from '../dto/barber.dto'
...
export class CreateNewBarberController {
  ...
  async create(@Body() body: BarberAndAccountDTO) {
    ...
    const barberAlreadyExists = await this.account.findIndexesCredentials(
      cpf,
      email,
      phone
    )
    ...
    if (barberAlreadyExists) {
      throw new ConflictException(
        'This account already exists, please do login.'
      )
    }

    console.log(
      JSON.stringify(`BARBER DATA:  ${JSON.stringify(barberData, null, 2)}`)
    )
    console.log(
      JSON.stringify(`ACCOUNT DATA:  ${JSON.stringify(accountData, null, 2)}`)
    )
